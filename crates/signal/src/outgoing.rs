...
use petunia_data::message::{Range, range};
...
mod tests {
    ...
    use petunia_data::message::project::{Fragment, classify};

    /// The plain-message half of `classify`. The receive path needs the
    /// reactions, edits and deletes `classify` keeps; a round-trip test does not.
    fn from_content(
        envelope: &presage::libsignal_service::content::Content,
    ) -> Option<(Thread, petunia_data::Message)> {
