...
/// Renders the body into spans. `revealed` lists the spoiler segments the reader
/// has clicked; anything else with `spoiler` set is drawn as a solid block.
pub fn spans<'a>(
    id: MessageId,
    body: &'a str,
    ranges: &[Range],
    ...
    segments(body, ranges)
        .into_iter()
        .map(|segment| {
            let styles = segment.styles;
            let hidden =
                styles.spoiler && !revealed.contains(&(id.timestamp, segment.start));
            let text = &body[segment.start..segment.end];

            // A hidden spoiler must not leak its text through glyph widths, so
            // it is replaced rather than merely recoloured. A mention carries a
            // placeholder in the body, so the name is substituted in.
            let mut span = match (hidden, styles.mention) {
                (true, _) => Span::new("█".repeat(text.chars().count().clamp(1, 40)))
                    .color(colors.muted)
                    .link(Link::Reveal(id, segment.start)),
                (false, Some(uuid)) => Span::new(format!("@{}", state.sender_name(uuid)))
                    .color(colors.accent)
                    .background(iced::Color {
                        a: 0.15,
                        ..colors.accent
                    }),
                (false, None) => Span::new(text),
            };

            if styles.bold {
                span = span.font(theme::FONT_BOLD);
            }
            if styles.italic {
                span = span.font(theme::FONT_ITALIC);
            }
            if styles.strikethrough {
                span = span.strikethrough(true);
            }
            if styles.monospace {
                span = span.font(theme::FONT_MONO).background(colors.sunken);
            }
            if styles.link && !hidden {
                span = span
                    .color(colors.accent)
                    .underline(true)
                    .link(Link::Url(text.to_string()));
            }
            span
        })
        .collect()
...
/// Byte ranges of anything that looks like a link. Hand-rolled: a regex crate
/// would be a dependency for thirty lines of scanning.
