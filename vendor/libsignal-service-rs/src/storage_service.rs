...
pub enum StorageServiceError {
    ...
    Invalid,
    /// Another device wrote a newer manifest first (HTTP 409). The caller
    /// should re-read the manifest, rebuild its change on top of it, and
    /// write again.
    #[error("storage service manifest conflict")]
    Conflict,
...
impl StorageService {
    ...
    pub async fn read_items(
        ...
        Ok(self
            .read_items_keyed(keys, record_ikm)
            .await?
            .into_iter()
            .map(|(_, record)| record)
            .collect())
    ...
    /// As [`Self::read_items`], but each record is paired with the item key it
    /// was stored under — which is what an update has to reference when it
    /// replaces one record in the manifest.
    pub async fn read_items_keyed(
        &self,
        keys: Vec<Vec<u8>>,
        record_ikm: Option<&[u8]>,
        ...
        items
            ...
            .map(|item| {
                Ok((
                    item.key.clone(),
                    Self::decrypt_item(&self.storage_key, item, record_ikm)?,
                ))
            })
            .collect()
    }

    /// Apply a [`WriteOperation`]: a new manifest plus the items it inserts
    /// and the keys it drops.
    ///
    /// UNVERIFIED: written from the Storage Service protocol as the official
    /// clients implement it (Signal-Android `StorageServiceApi.writeStorageRecords`),
    /// not from a captured exchange. The endpoint (`PUT /v1/storage`), the
    /// protobuf content type and the 409-carries-the-current-manifest response
    /// are all inferred rather than observed here. A manifest written at the
    /// wrong version, or one that drops identifiers, rewrites the account's
    /// synced state on every linked device.
    pub async fn write(
        &self,
        operation: WriteOperation,
    ) -> Result<(), StorageServiceError> {
        let mut buf = Vec::with_capacity(operation.encoded_len());
        operation.encode(&mut buf).expect("infallible encode into Vec");
        ...
        // The conflict body is a protobuf StorageManifest, not the JSON that
        // the generic status handling would try to parse out of a 409.
