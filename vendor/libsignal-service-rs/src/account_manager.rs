...
impl AccountManager {
    ...
    >(
        ...
        let retain_avatar = matches!(avatar, AvatarWrite::RetainAvatar);
        let encrypted_avatar = match avatar {
            AvatarWrite::NewAvatar(reader) => {
                let mut plaintext = Vec::new();
                reader
                    .read_to_end(&mut plaintext)
                    .map_err(ServiceError::from)?;
                Some(profile_cipher.encrypt_avatar(plaintext, csprng)?)
            },
            AvatarWrite::RetainAvatar | AvatarWrite::NoAvatar => None,
        };
        let mut encrypted_avatar = encrypted_avatar.map(std::io::Cursor::new);
        let avatar = match &mut encrypted_avatar {
            Some(cursor) => AvatarWrite::NewAvatar(cursor),
            None if retain_avatar => AvatarWrite::RetainAvatar,
        ...
        Ok(self
            .websocket
            .write_profile::<std::io::Cursor<Vec<u8>>, S>(
                &profile_key_version,
                &name,
                &about,
                &about_emoji,
