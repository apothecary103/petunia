...
impl ProfileCipher {
    ...
    /// Encrypts a raw avatar image for upload to the profile CDN.
    ///
    /// The inverse of [`Self::decrypt_avatar`]: a random 12-byte nonce
    /// followed by the AES-256-GCM ciphertext under the profile key. Unlike a
    /// name or an about, an avatar carries no padding brackets — Signal's
    /// clients resize the image instead — so the plaintext is encrypted as-is.
