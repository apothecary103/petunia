...
impl PushService {
    ...
    /// Creates the group `group` describes.
    ///
    /// The credentials are the group's own, derived from its master key, not
    /// the account's: the server is told which group is being written to by
    /// what the request is authorised with and by nothing else.
    ///
    /// UNVERIFIED: modelled on Signal-Android's `GroupsV2Api.putNewGroup`. Not
    /// exercised against a live server from this tree.
