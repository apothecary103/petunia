...
/// The usernames to offer the server for `wanted`.
///
/// A discriminator the caller chose is honoured as the only candidate: asking
/// for `yuki.01` and being given `yuki.42` because the first was taken is not a
/// reservation anybody wants silently. A bare nickname takes what it is given.
fn candidates_for(
    wanted: &str,
) -> Result<Vec<usernames::Username>, ServiceError> {
    if let Ok(chosen) = usernames::Username::new(wanted) {
        return Ok(vec![chosen]);
    }

    usernames::Username::candidates_from(
        &mut rand::rng(),
        wanted,
        usernames::NicknameLimits::default(),
    )
    .map_err(|_e| ServiceError::InvalidFrame {
        reason: "nickname is not a valid username nickname",
    })?
    .iter()
    .map(|candidate| usernames::Username::new(candidate))
    .collect::<Result<Vec<_>, _>>()
    .map_err(|_e| ServiceError::InvalidFrame {
        reason: "generated username candidate is invalid",
    })
}
...
/// A username hash the server has reserved for this account.
///
/// The reservation is short-lived; it has to be turned into the account's
/// actual username with [`SignalWebSocket::confirm_username`].
#[derive(Debug)]
pub struct ReservedUsername {
    /// The discriminated username the server picked out of the candidates.
    pub username: usernames::Username,
    /// Its hash, as the server echoed it back.
    pub hash: Vec<u8>,
}

impl SignalWebSocket<Identified> {
    /// Reserves a username hash for `nickname`.
    ///
    /// A Signal username is a nickname and a discriminator -- `yuki.01` -- and
    /// `nickname` may be either. Given a bare nickname, candidate discriminators
    /// come from the `usernames` crate itself
    /// ([`usernames::Username::candidates_from`]), which is what the official
    /// clients use; the server picks the first free one and echoes its hash
    /// back, which is matched against the candidates to recover the full
    /// `nickname.discriminator`. Given a discriminator too, that one username is
    /// the only candidate offered, so it is taken or the reservation fails.
    ///
    /// UNVERIFIED: the request and response shapes below are modelled on
    /// Signal-Server's `ReserveUsernameHashRequest`/`Response` and have not
    /// been exercised against a live server from this tree. Test against a
    /// real account before relying on it.
    pub async fn reserve_username(
        &mut self,
        nickname: &str,
    ) -> Result<ReservedUsername, ServiceError> {
        #[derive(Serialize)]
        ...
        let candidates = candidates_for(nickname)?;

        let response: ReserveUsernameResponse = self
            .http_request(Method::PUT, "/v1/accounts/username_hash/reserve")?
            .send_json(ReserveUsernameRequest {
                username_hashes: candidates
                    .iter()
                    .map(|c| BASE64_URL_SAFE_NO_PAD.encode(c.hash()))
                    .collect(),
            })
            .await?
        ...
        let username = candidates
            .into_iter()
            .find(|c| c.hash().as_slice() == response.username_hash.as_slice())
            .ok_or(ServiceError::InvalidFrame {
                reason: "server reserved a hash that was not offered",
            })?;

        Ok(ReservedUsername {
            username,
            hash: response.username_hash,
        })
    }

    /// Confirms a reservation made by [`Self::reserve_username`], making
    /// `username` the account's username.
    ///
    /// Proves knowledge of the nickname and discriminator behind the reserved
    /// hash, and stores the encrypted username the link will serve.
    ///
    /// UNVERIFIED: see [`Self::reserve_username`].
    pub async fn confirm_username(
        ...
        let zk_proof = username.proof(&randomness).map_err(|_e| {
            ServiceError::InvalidFrame {
                reason: "could not prove username",
            }
        })?;

        let (_entropy, encrypted_username) = usernames::create_for_username(
            &mut rand::rng(),
            username.to_string(),
            None,
        )
    ...
    /// Clears the account's username and, with it, its username link.
    ///
    /// UNVERIFIED: see [`Self::reserve_username`].
