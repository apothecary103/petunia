...
impl<C: CredentialsCache> GroupsManager<C> {
    ...
    /// Creates a group with `title` and `members` in it, under a master key the
    /// caller has already chosen.
    ///
    /// The master key is the group's identity, and it is generated rather than
    /// assigned: the server never sees it, only the public parameters derived
    /// from it, which is what makes the group's title and membership opaque to
    /// it. Everybody who is to be in the group has to be told the master key
    /// separately, which is what the `GroupContextV2` on the first message does.
    ///
    /// A candidate with a credential joins as a member; one without is invited
    /// and joins when they accept.
    ///
    /// UNVERIFIED: not exercised against a live server from this tree.
    pub async fn create_group<R: Rng + CryptoRng>(
        &mut self,
        csprng: &mut R,
        master_key: GroupMasterKey,
        title: &str,
        self_credential: &zkgroup::profiles::ExpiringProfileKeyCredential,
        members: &[super::GroupMemberCandidate],
    ) -> Result<(), ServiceError> {
        let group_secret_params =
            GroupSecretParams::derive_from_master_key(master_key);
        let group = GroupOperations::new(group_secret_params)
    ...
    /// Somebody's expiring profile key credential, for putting them in a group.
    pub async fn member_credential(
        &mut self,
