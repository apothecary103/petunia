...
pub enum Error<S: std::error::Error> {
    ...
    StorageService(#[from] libsignal_service::StorageServiceError),
    #[error("no master key: cannot derive the storage service key")]
    NoMasterKey,
    #[error("no storage service record for this contact")]
    UnknownStorageRecord,
}
